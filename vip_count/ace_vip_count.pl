...
open (OUTPUTFILERUN, ">output") or die "Unable To Write File outputfile, $^E\n";

my $userlogin = `whoami`;
chomp $userlogin;
my $web = ".web";
print "Loging with $userlogin$web\n";
print ".Web Password: ";
ReadMode('noecho');
...
foreach my $ace (<HOSTFILE>) {
        ...
        my $cmd = new Expect;
        my $command = "ssh -2 -l $userlogin$web -o PubkeyAuthentication=no -o StrictHostKeyChecking=no -o NumberOfPasswordPrompts=1 $ace";
