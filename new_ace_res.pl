...
foreach my $ace (<HOSTFILE>) {
		...
		open (OUTPUTTOTAL, ">$ace$fname.csv") or die "Unable to open $ace$fname.csv, $^E\n";
...
foreach my $l (<RSFILE>) {
        ...
        if ($l =~ /Resource/) {
				...
				print OUTPUTTOTAL " ,Peak Bytes,Peak Bits,Peak Gbps,Current Bytes,Current Bits,Current Gbps,Denied";
                print OUTPUT "\n";
                }
        if ($l =~ /^Context/) {
                print OUTPUT "$l";
        ...
        if ($l =~ /bandwidth/) {
				...
				print OUTPUTTOTAL "Bandwidth,$pbybandwidth,$pbibandwidth,$pgbpsbandwidth,$cbybandwidth,$cbibandwidth,$cgbpsbandwidth,$deny\n";
                }
        if ($l =~ /ssl-connections rate/) {
				...
				print OUTPUTTOTAL "SSL-Connections rate,$pssl, , ,$cssl, , ,$ssl_deny\n";
                }
                }
                system ("rm $pwd\/$ace");
