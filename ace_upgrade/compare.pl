...
foreach my $ace (<HOSTFILE>) {
        ...
        open (OUTPUTFILERUN, ">$ace$run") or die "Unable To Write File $ace$run, $^E\n";
        ...
        my @backuprun=`cat backup/$ace$run`;
        my @run=`cat $ace$run`;
        open (FILE3, ">file3") or die "Unable To Write File3, $^E\n";
        open (FILE4, ">file4") or die "Unable To Write File4, $^E\n";
        print FILE3 @backuprun;
        print FILE4 @run;
        system ("diff -a --suppress-common-lines file3 file4 >>$ace.csv");
