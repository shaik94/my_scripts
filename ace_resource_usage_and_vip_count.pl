...
my $vip_file ="_output.csv";
open (HOSTFILE, "<$hostfile") or die "Unable to open $hostfile, $^E\n";
open (OUTPUTSIN, ">$outfile$vip_file") or die "Unable to open $outfile$vip_file, $^E\n";
...
foreach my $ace (<HOSTFILE>) {
        ...
        my $cmd = new Expect;
        my $command = "ssh -2 -l admin -o PubkeyAuthentication=no -o StrictHostKeyChecking=no -o NumberOfPasswordPrompts=1 $ace1[0]";
        ...
        $cmd->expect(15, "$ace1[0]/$context_name");
        push (@configs, $cmd->before);
                open (VIPINCON, ">vip_count") or die "Unable To Write File vip_count, $^E\n";
        print VIPINCON @configs;
        sleep (1);
        @configs = ();
...
system ("rm $pwd\/ace$fname.csv");
        my $vips_count = `cat vip_count | grep -c SRVC`;
                print OUTPUTSIN "VIP count :, $vips_count\n";
                        }
